...
def Generator(norm="layer"):
    ...
    downsample(128, 4, norm=norm),  # (batch_size, 64, 64, 128)
    downsample(256, 4, norm=norm),  # (batch_size, 32, 32, 256)
    downsample(512, 4, norm=norm),  # (batch_size, 16, 16, 512)
    downsample(512, 4, norm=norm),  # (batch_size, 8, 8, 512)
    ...
    upsample(256, 4, norm=norm),  # (batch_size, 32, 32, 512)
    upsample(128, 4, norm=norm),  # (batch_size, 64, 64, 256)
    upsample(64, 4, norm=norm),  # (batch_size, 128, 128, 128)
    ]
    ...
    for up, skip in zip(up_stack, skips):
        x = up(x)
        x, _ = UAttention(filt[nn], name=f"U_atte{nn}")(x, skip)
        nn += 1
...
def Discriminator(norm="layer"):
    ...
    down1 = downsampleSN(64, 4, False, act = "LReLU")(x)  # (batch_size, 128, 128, 64)

    x = ResModPreActSN(64, 4, norm=norm, act = "LReLU", name="res0")(down1)
    
    
    down2 = downsampleSN(128, 4, norm = norm, act = "LReLU")(x)  # (batch_size, 64, 64, 128)

    x = ResModPreActSN(128, 4, act = "LReLU", norm=norm, name="res1")(down2)

    x, _ = SelfAttentionSN(128, act = "LReLU", name = "attention_0")(x)
    
    down3 = downsampleSN(256, 4, norm=norm, act = "LReLU" )(x)  # (batch_size, 32, 32, 256)

    #conv = tf.keras.layers.Conv1D(512, 4, strides=1,
    #                            kernel_initializer=initializer,
    #                            use_bias=False)(down3)  # (batch_size, 31, 31, 512)
    x = ResModPreActSN(256, 4, norm=norm, act = "LReLU",  name="res2")(down3)
    
    x, _ = SelfAttentionSN(256, act = "LReLU", name = "attention_1")(x)
    #if norm=="batch":
    ...
    last = tf.keras.layers.Conv1D(1, 4, strides=1,
                                kernel_initializer=initializer,
                                 activation=None)(x)#(batchnorm1)  # (batch_size, 30, 30, 1)
