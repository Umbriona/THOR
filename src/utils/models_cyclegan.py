...
from utils import models_gan_atte2 as gan
...
class CycleGan(tf.keras.Model):
    ...
    @tf.function
    def train_step_bert(self, batch_data):

        with tf.GradientTape(persistent=True) as tape:
            real_x, _, prob_x, _, W_x = batch_data[0]
            real_y, _, prob_y, _, W_y= batch_data[1]
            ...
            fake_y = self.G(input_x_real, training=True)   # G:x -> y'
            fake_y = tf.math.multiply(fake_y, mask_x) # Preserve padding
            input_y_fake = tf.math.add(fake_y, prob_x)
            cycled_x = self.F(input_y_fake, training=True) # Cycle: F:y' -> x
            
            fake_x = self.F(input_y_real, training=True)   # F:y -> x'
            fake_x = tf.math.multiply(fake_x, mask_y) ##Apply mask
            input_x_fake = tf.math.add(fake_x, prob_y)
            cycled_y = self.G(input_x_fake, training=True) # Cycle: G:x' -> y        

            # Identity mapping
            same_x = self.F(input_x_real, training=True)  #F:x -> x
            same_y = self.G(input_y_real, training=True)  #G:y -> y
    ...
    @tf.function
    def validate_step(self, x):#batch_data):
        ...
        return temp_diff_y, temp_diff_x#diff_x, diff_y
    ...
    def generate_step(self, batch_data):
        ...
        fake_y = self.G(X_bin, training=False)
        fake_x = self.F(Y_bin, training=False)
        seqs = []
