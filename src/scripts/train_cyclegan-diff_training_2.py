...
def train(config, model, data, time, classifyer):
   ...
   # loss_writer_cycle = tf.summary.create_file_writer(os.path.join(base_dir,'loss_param_cycle'))
    ...
    for epoch in range(config['CycleGan']['epochs']):
        ...
        for step, x in enumerate(zip(batches_x,batches_y)):
         ...
         #       tf.profiler.experimental.start(os.path.join(base_dir, "profile"), options=tf.profiler.experimental.ProfilerOptions(
         #           host_tracer_level=2,
        ...
        print(f"Time for validation steps: {stop_time - start_time}")
            ...
            #    tf.summary.scalar('temp_diff', model.metricss["temp_diff_x"].result(), step=epoch, description = 'temp_diff_x')
            #with temp_diff_summary_y.as_default():
            #    tf.summary.scalar('temp_diff', model.metricss["temp_diff_y"].result(), step=epoch, description = 'temp_diff_y')
        ...
        if epoch % 10 == 0:
            val_x = data['meso_val'].batch(config['CycleGan']['batch_size'], drop_remainder=True).prefetch(buffer_size=20)
            val_y = data['thermo_val'].batch(config['CycleGan']['batch_size'], drop_remainder=True).prefetch(buffer_size=20)
   ...
   #             tf.summary.scalar('loss', model.metricss['loss_G'].result(), step = epoch, description = 'X transform')
   #             tf.summary.scalar('acc', model.metricss['acc_x'].result(), step = epoch, description = 'X transform' )


    #    with F_summary_writer.as_default():
    #        tf.summary.scalar('loss', model.metricss['loss_F'].result(), step = epoch, description = 'Y transform')
    #        tf.summary.scalar('acc', model.metricss['acc_y'].result(), step = epoch, description = 'Y transform' )

    #    with D_x_summary_writer.as_default():         
    #        tf.summary.scalar('loss_discriminator', model.metricss['loss_disc_y'].result(), step = epoch, description = 'X discriminator')        
    #    with D_y_summary_writer.as_default():        
    #        tf.summary.scalar('loss_discriminator', model.metricss['loss_disc_x'].result(), step = epoch, description = 'Y discriminator')    
    #    with X_c_summary_writer.as_default(): 
    #        tf.summary.scalar('loss_cycle', model.metricss['loss_cycle_x'].result(), step = epoch, description = 'X cycle')
    #        tf.summary.scalar('acc', model.metricss['cycled_acc_x'].result(), step = epoch, description = 'X cycle' )         
    #    with Y_c_summary_writer.as_default():
    #        tf.summary.scalar('loss_cycle', model.metricss['loss_cycle_y'].result(), step = epoch, description = 'Y cycle')
    #        tf.summary.scalar('acc', model.metricss['cycled_acc_y'].result(), step = epoch, description = 'Y cycle' )
            
    #    with X_id_summary_writer.as_default(): 
    #        tf.summary.scalar('loss_cycle', model.metricss['loss_id_x'].result(), step = epoch, description = 'X Id')
    #        tf.summary.scalar('acc', model.metricss['id_acc_x'].result(), step = epoch, description = 'X Id' )         
    #    with Y_id_summary_writer.as_default():
    #        tf.summary.scalar('loss_cycle', model.metricss['loss_id_y'].result(), step = epoch, description = 'Y Id')
    #        tf.summary.scalar('acc', model.metricss['id_acc_y'].result(), step = epoch, description = 'Y Id' )
